...
export async function POST(req: NextRequest) {
  ...
  // Truncate untrusted caller-supplied strings before interpolation into the AI prompt
  const videoTitle = rawTitle.slice(0, 200);
  const studyGuide: StudyGuide = {
    summary: (rawGuide.summary ?? '').slice(0, 500),
    coreInsights: (rawGuide.coreInsights ?? []).slice(0, 20).map(s => s.slice(0, 200)),
    keyConcepts: (rawGuide.keyConcepts ?? []).slice(0, 20).map(c => ({
